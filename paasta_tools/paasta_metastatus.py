...
def verify_marathon_clients_and_return_framework_ids(marathon_clients):
    ids = []
    unreachable_clients = []
    for client in marathon_clients:
        try:
            ids.append(client.get_info().framework_id)
        except MarathonError:
            ids.append(str(len(ids)))
            unreachable_clients.append(client)
    marathon_clients[:] = (c for c in marathon_clients if c not in unreachable_clients)
    return ids
